...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: "#{proxy} yum -y install gcc gcc-c++ curl-devel openssl-devel zlib-devel mariadb mariadb-server mysql-devel sqlite-devel vim vim-enhanced git svn patch libyaml-devel libffi-devel autoconf patch readline-devel automake libtool bison"
  config.vm.provision "shell", inline: "#{proxy} wget http://nodejs.org/dist/v0.11.13/node-v0.11.13-linux-x64.tar.gz"
  config.vm.provision "shell", inline: "tar --strip-components 1 -xzvf node-v0.11.13-linux-x64.tar.gz -C /usr/local"
  config.vm.provision "shell", inline: "#{proxy} curl -sSL https://get.rvm.io | #{proxy} bash -s -- --autolibs=read-fail --ruby"
  ...
  config.vm.provision "shell", inline: "#{proxy} rvm alias create cedek ruby-2.1.3@cedek"
